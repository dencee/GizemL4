...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public WorldPanel(int w, int h, int cpr) {
		...
		for (int col = 0; col < cells.length; col++) {
			for (int row = 0; row < cells.length; row++) {
				cells[col][row] = new Cell(col * cellSize, row * cellSize, cellSize);
			}
		}
	}
	...
	public int getLivingNeighbors(Cell[][] cells, int cellRow, int cellCol) {
		...
		if (cellRow != 0) {
			if (cells[cellRow - 1][cellCol].isAlive)
				livingNeighbors++;
		}
		...
		if (cellCol != 0) {
			if (cells[cellRow][cellCol - 1].isAlive)
				livingNeighbors++;
		}
		...
		if (cellRow != 0 && cellCol != 0) {
			if (cells[cellRow - 1][cellCol - 1].isAlive)
				livingNeighbors++;
		}
	...
	public void mousePressed(MouseEvent e) {
		...
		int x = e.getX();
		int y = e.getY();
		Cell cellPressed = cells[x/cellSize][y/cellSize];
		cellPressed.isAlive = true;
